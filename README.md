...
```bash
./start.sh              # 启动 Agent 服务（默认）
./start.sh mcp          # 启动 MCP Server
./start.sh --mcp-notify # 启动 Agent 并检查 MCP 配置是否就绪
```

**Windows：**

```bat
start.bat               REM 启动 Agent 服务（默认）
start.bat mcp           REM 启动 MCP Server
start.bat --mcp-notify  REM 启动 Agent 并检查 MCP 配置是否就绪
```
...
| 参数 | 说明 |
|------|------|
| `agent`（默认） | 启动 Agent 服务（飞书 → Claude） |
| `mcp` | 启动 MCP Server（Claude → 飞书） |
| `--mcp-notify` | 启动前检查 MCP 飞书通知是否已配置 |
| `-h` / `--help` | 显示帮助信息 |
