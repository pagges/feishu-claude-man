...
def run_cases(base_url: str, access_key: str, secret_key: str,
              project_id: str, case_ids: list, report_name: str,
              resource_pool_id: str = None, environment_id: str = None):
    """执行测试用例"""
    url = f"{base_url}/api/api/testcase/batch/run"
...
def get_report(base_url: str, access_key: str, secret_key: str, report_id: str):
    """获取测试报告"""
    url = f"{base_url}/api/api/definition/report/getReport/{report_id}"
...
def wait_for_report(base_url: str, access_key: str, secret_key: str,
    ...
    while True:
        ...
        if status in ["Success", "Error", "Completed", "success", "error"]:
            return report
...
def main():
    parser = argparse.ArgumentParser(description="Run MeterSphere test cases")
    ...
    case_ids = [id.strip() for id in args.ids.split(",") if id.strip()]
    report_name = args.report_name or f"Case Run {time.strftime('%Y-%m-%d %H:%M:%S')}"

    try:
        report_id = run_cases(
            args.base_url, args.access_key, args.secret_key,
            args.project_id, case_ids, report_name,
            args.resource_pool_id, args.environment_id
        )
